...
@section('content')
    <div class="rounded-lg bg-gray-800 p-5 w-full font-sans min-h-80 bg-opacity-30 text-white">
            ...
            <form action="{{ route('usersManagement.update', $users->id) }}" method="post">
                ...
                <div class="space-y-2 font-sans">
                    <div class="text-gray-600">
                        ...
                        <input type="text" name="name" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('name') is-invalid @enderror"  placeholder="Input name" value="{{ $users->name }}" required autofocus>
                        @error('name')
                    ...
                    <div class="text-gray-600">
                        ...
                        <input type="email" name="email" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('email') is-invalid @enderror"  placeholder="Input email" value="{{ $users->email }}" required>
                        @error('email')
                            <p class="text-red-600 font-bold text-[12px]">{{ $message }}</p>
                        @enderror
                    </div>
